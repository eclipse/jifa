...
public class TestGlobalDiagnoser {
    ...
    public void testDiagnoseBasic() throws Exception {
        ...
        Assertions.assertArrayEquals(seriousProblems.get(METASPACE_FULL_GC.getName()).stream().mapToDouble(d -> d).toArray(),
        ...
        Assertions.assertArrayEquals(seriousProblems.get(HEAP_MEMORY_FULL_GC.getName()).stream().mapToDouble(d -> d).toArray(),
                new double[]{95765}, DELTA);

        MostSeriousProblemSummary mostSeriousProblem = diagnose.getMostSeriousProblem();
        Assertions.assertEquals(mostSeriousProblem.getProblem(), new I18nStringView("jifa.gclog.diagnose.abnormal.metaspaceFullGC"));
        Assertions.assertTrue(mostSeriousProblem.getSuggestions().contains(new I18nStringView("jifa.gclog.diagnose.suggestion.checkMetaspace")));
        Assertions.assertTrue(mostSeriousProblem.getSuggestions().contains(new I18nStringView("jifa.gclog.diagnose.suggestion.enlargeMetaspace")));
        Assertions.assertFalse(mostSeriousProblem.getSuggestions().contains(new I18nStringView("jifa.gclog.diagnose.suggestion.upgradeTo11G1FullGC")));
    ...
    public void testNoProblem() throws Exception {
        ...
        Assertions.assertEquals(diagnose.getSeriousProblems().size(), 0);
        Assertions.assertNull(diagnose.getMostSeriousProblem());
    }
    ...
    public void testOrderSites() throws Exception {
        ...
        MostSeriousProblemSummary mostSeriousProblem = diagnose.getMostSeriousProblem();
        Assertions.assertEquals(mostSeriousProblem.getProblem(), new I18nStringView("jifa.gclog.diagnose.abnormal.longYoungGCPause"));
        Assertions.assertTrue(mostSeriousProblem.getSuggestions().contains(new I18nStringView("jifa.gclog.diagnose.suggestion.shrinkYoungGen")));

        List<TimeRange> sites = mostSeriousProblem.getSites();
        Assertions.assertEquals(sites.size(), 3);
        double[] actual = sitesToArray(sites);
        Assertions.assertArrayEquals(actual, new double[]{0, 54204 + 442.2627, 174204, 304204 + 442.2627, 374204, 444204 + 442.2627}, DELTA);
    }
    ...
    public void testDiagnoseZGC() throws Exception {
        ...
        Assertions.assertArrayEquals(seriousProblems.get(ALLOCATION_STALL.getName()).stream().mapToDouble(d -> d).toArray(),
                new double[]{7777 - 0.204, 7888 - 0.391}, DELTA);
        Assertions.assertArrayEquals(seriousProblems.get(OUT_OF_MEMORY.getName()).stream().mapToDouble(d -> d).toArray(),
                new double[]{7889}, DELTA);

        MostSeriousProblemSummary mostSeriousProblem = diagnose.getMostSeriousProblem();
        Assertions.assertEquals(mostSeriousProblem.getProblem(), new I18nStringView("jifa.gclog.diagnose.abnormal.outOfMemory"));
        Assertions.assertTrue(mostSeriousProblem.getSuggestions().contains(new I18nStringView("jifa.gclog.diagnose.suggestion.checkMemoryLeak")));
        Assertions.assertTrue(mostSeriousProblem.getSuggestions().contains(new I18nStringView("jifa.gclog.diagnose.suggestion.enlargeHeap")));
