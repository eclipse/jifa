...
public class TestEventDiagnoser {
    ...
    public void testDiagnoseBasic() throws Exception {
        ...
        Assertions.assertEquals(eventDiagnoseInfos.length, model.getAllEvents().size());
        ...
        Assertions.assertEquals(hit, 2);
    }

    private void absContainExactTypes(EventAbnormalSet abs, List<AbnormalType> types) {
        Assertions.assertEquals(abs.size(), types.size());
        for (AbnormalType type : types) {
            Assertions.assertTrue(abs.contains(type));
        }
    }
