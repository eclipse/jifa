...
class HistogramRoute extends HeapBaseRoute {
        ...
        try {
            ...
            switch (groupingBy.gb) {
                case BY_CLASS: {
                    ...
                    future.complete(buildClassHistogramRecord(snapshot, (List<? extends ClassHistogramRecord>) histogram.getClassHistogramRecords(), ascendingOrder, sortBy, searchText, searchType, pagingRequest));
                    break;
                }
                case BY_CLASSLOADER: {
                    Histogram.ClassLoaderTree tree = (Histogram.ClassLoaderTree) result;
                    future.complete(buildClassLoaderHistogramRecord(snapshot, (List<? extends XClassLoaderHistogramRecord>) tree.getElements(), ascendingOrder, sortBy, searchText, searchType, pagingRequest));
                    break;
                }
                case BY_SUPERCLASS: {
                    Histogram.SuperclassTree tree = (Histogram.SuperclassTree) result;
                    future.complete(buildSuperClassHistogramRecord(tree, tree.getElements(), ascendingOrder, sortBy, searchText, searchType, pagingRequest));
                    break;
                }
                case BY_PACKAGE: {
                    Histogram.PackageTree tree = (Histogram.PackageTree) result;
                    future.complete(buildPackageHistogramRecord(tree, tree.getElements(), ascendingOrder, sortBy, searchText, searchType, pagingRequest));
                    break;
                }
                default: {
        ...
        try {
            ...
            switch (groupingBy.gb) {
                    ...
                    for (Object elem : elems) {
                        if (elem instanceof XClassLoaderHistogramRecord) {
                            if (((XClassLoaderHistogramRecord) elem).getClassLoaderId() == parentObjectId) {
                                children = (List<? extends ClassHistogramRecord>) ((XClassLoaderHistogramRecord) elem).getClassHistogramRecords();
                                break;
                            }
                        }
                    }
                    if (children != null) {
                        future.complete(buildClassHistogramRecord(snapshot, children, ascendingOrder, sortBy, null, null, pagingRequest));
                    } else {
                ...
                case BY_SUPERCLASS: {
                    ...
                    if (children != null) {
                        future.complete(buildSuperClassHistogramRecord(tree, children, ascendingOrder, sortBy, null, null, pagingRequest));
                    } else {
                        future.complete(PageViewBuilder.build(new ArrayList<>(), pagingRequest));
                    }
                    break;
                ...
                case BY_PACKAGE: {
                    ...
                    if (targetParentNode != null) {
                        ...
                        future.complete(buildPackageHistogramRecord(tree, elems, ascendingOrder, sortBy, null, null, pagingRequest));
                    } else {
                        future.complete(PageViewBuilder.build(new ArrayList<>(), pagingRequest));
                    }
                    break;
    ...
    private PageView<Record> buildClassHistogramRecord(ISnapshot snapshot, List<? extends ClassHistogramRecord> records, boolean ascendingOrder, String sortBy, String searchText, SearchType searchType, PagingRequest pagingRequest) {
    ...
    private PageView<Record> buildClassLoaderHistogramRecord(ISnapshot snapshot, List<? extends XClassLoaderHistogramRecord> records, boolean ascendingOrder, String sortBy, String searchText, SearchType searchType, PagingRequest pagingRequest) {
    ...
    private PageView<Record> buildSuperClassHistogramRecord(Histogram.SuperclassTree tree, List<?> records, boolean ascendingOrder, String sortBy, String searchText, SearchType searchType, PagingRequest pagingRequest) {
    ...
    private PageView<Record> buildPackageHistogramRecord(Histogram.PackageTree tree, List<?> records, boolean ascendingOrder, String sortBy, String searchText, SearchType searchType, PagingRequest pagingRequest) {
        PageViewBuilder<?, Record> builder = PageViewBuilder.fromList(records);
        return builder
                .paging(pagingRequest)
                .map(e -> {
