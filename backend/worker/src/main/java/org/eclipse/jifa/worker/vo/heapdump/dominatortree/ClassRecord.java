...
public class ClassRecord extends BaseRecord implements Searchable {
    private static Map<String, Comparator> sortTable = new SortTableGenerator()
            .add("id", ClassRecord::getObjectId)
            .add("shallowHeap", ClassRecord::getShallowSize)
            .add("retainedHeap", ClassRecord::getRetainedSize)
            .add("percent", ClassRecord::getPercent)
            .add("Objects", ClassRecord::getObjects)
            .build();
    private int objects;

    public static Comparator<ClassRecord> sortBy(String field, boolean ascendingOrder) {
