 ...
 ********************************************************************************/
{
    ...
    function Frame(flameGraph, raw, depth, isRoot = false) {
        ...
        this.drawSelf = function () {
            ...
            if (this.width > this.fg.$showTextWidthThreshold && this.text.length > 0) {
                ...
                if (w <= leftW) {
                    this.fg.$context.fillText(this.text, this.x + this.fg.$textGap, this.y + this.height / 2 + 1)
                    this.visibleText = this.text
                } else {
                    // truncate text and append suffix
                    let len = Math.floor(this.text.length * (leftW - this.fg.$context.measureText(this.fg.$moreText).width) / w)
                    let text = null
                    for (let i = len; i > 0; i--) {
                    ...
                    if (text != null) {
                        this.fg.$context.fillText(text, this.x + this.fg.$textGap, this.y + this.height / 2 + 1)
                    }
        ...
        this.draw = function (x, y, w, h) {
            ...
            if (this.children) {
                ...
                if (!this.fg.$pinned || this === this.fg.$pinnedFrame || this.fg.$drawingChildrenOfPinnedFrame) {
                    ...
                    if (this.fg.$drawingChildrenOfSideFrame) {
                        if (!this.fg.$drawingLeftSide || this.selfWeight === 0) {
                            for (let i = 0; i < this.children.length; i++) {
                                if (this.fg.$drawingLeftSide && i === this.children.length - 1 ||
        ...
        this.touch = function (x, y) {
            ...
            if (this !== this.fg.$root) {
                ...
                // title
                this.fg.$framePostcardContentMainTitle.innerText = this.fg.$$titleGenerator(this.fg.$dataSource, this.raw, this.infomation)
                this.fg.$framePostcardContentMainLine.style.background = 'linear-gradient(' + direction + ', ' + hexToRGB(this.color[1], 0.7) + ' 0% ' + hp + '%, ' + hexToRGB(this.color[1], .2) + ' ' + hp + '% 100%)'
                ...
                if (details) {
                    ...
                    let content = null
                    if (keys.length > 0) {
                        content = '<div id ="frame-postcard-content-main-details" style="width: 100%; font-size: 11px; word-wrap: break-word">'
                ...
                // foot
                this.fg.$framePostcardContentFoot.innerText = this.fg.$$footTextGenerator(this.fg.$dataSource, this.raw, this.infomation)
                ...
                this.fg.$framePostcardContentFoot.style.background = 'linear-gradient(to right, ' + hexToRGB(startColor) + ' 0% ' + wp + '%, ' + hexToRGB(endColor) + ' ' + wp + '% 100%)'
    ...
    class FlameGraph extends HTMLElement {
        ...
        render(reInitRenderContext, reGenFrames) {
            ...
            if (this.downward) {
                this.$root.draw(this.$fgHGap, this.$fgVGap, rect.width - this.$fgHGap * 2, this.$frameHeight)
            } else {
                this.$root.draw(this.$fgHGap, rect.height - this.$fgVGap - this.$frameHeight, rect.width - this.$fgHGap * 2, this.$frameHeight)
            }
            ...
            if (this.$stackTraceMaxDrawnDepth < this.$stackTraceMaxDepth) {
                ...
                if (this.downward) {
                    this.$currentScrollTopLimit = Math.max(height - this.$flameGraphInner.getBoundingClientRect().height, this.$flameGraphInner.scrollTop)
                } else {
                    this.$currentScrollTopLimit = Math.min(this.$flameGraphHeight - height, this.$flameGraphInner.scrollTop)
                }
            }
        }
        ...
        get stackTracesCounter() {
            return this._('stackTracesCounter',
                (dataSource) => this.$$dataExtractor(dataSource).length
            )
        }

        get stackTraceExtractor() {
            return this._('stackTraceExtractor',
        ...
        get rootFramesCounter() {
            return this._('rootFramesCounter',
                dataSource => this.$$dataExtractor(dataSource).length / this.$$frameStepper(dataSource, null)
            )
        }

        get rootFrameExtractor() {
            return this._('rootFrameExtractor',
                (dataSource, index) => {
        ...
        get childFramesCounter() {
            return this._('childFramesCounter',
                (dataSource, frame) => {
                    return frame[this.$$childFramesIndexer(dataSource, frame)].length / this.$$frameStepper(dataSource, frame)
                })
        }
        ...
        get childFramesIndexer() {
            return this._('childFramesIndexer',
                this.$$diff
                    ?
                    (dataSource, frame) => 5
                    :
        ...
        get weightsExtractor() {
            return this._('weightsExtractor',
                this.$$isLineFormat
                    ?
                    this.$$diff
                        ?
        ...
        floatColorToHexColor(float) {
            return '#' + this.floatToHex(float[0] * 255) + this.floatToHex(float[1] * 255) + this.floatToHex(float[2] * 255)
        }
        ...
        findFrame(x, y) {
            ...
            if (this.downward) {
                ...
                index = Math.floor((this.$root.y + this.$frameHeight - y) / (this.$frameHeight + this.$yGap))
            }
        ...
        decideFramePostcardLayout() {
            ...
            let showAtLeft = rect.left + 392 > (window.innerWidth || document.documentElement.clientWidth)
            if (showAtLeft) {
                this.$framePostcardContentMain.style.marginLeft = (366 - this.$framePostcardContentMain.clientWidth) + 'px'
                this.$framePostcardContent.style.left = '-392px'
                if (showAtTop) {
                    this.$framePostcardConnectingLine.style.transform = 'rotate(135deg) translate3d(0px, -.5px, 0)'
                } else {
                    this.$framePostcardConnectingLine.style.transform = 'rotate(-135deg) translate3d(0px, -.5px, 0)'
                }
            } else {
                ...
                if (showAtTop) {
                    this.$framePostcardConnectingLine.style.transform = 'rotate(45deg) translate3d(0px, -.5px, 0)'
                } else {
                    this.$framePostcardConnectingLine.style.transform = 'rotate(-45deg) translate3d(0px, -.5px, 0)'
                }
            }
