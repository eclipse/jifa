  ...
  jifa: {
    ...
    gclog: {
      ...
      memoryStats: {
        ...
        usedAvgAfterFullGCHint: "用于估算这个区域大概有多少常驻的对象，如果这个值较高，说明应用在该区域存在内存泄漏的可能性",
        usedAvgAfterOldGC: "Old GC后平均使用量",
        usedAvgAfterOldGCHint: "用于估算这个区域大概有多少常驻的对象，如果这个值较高，说明应用在该区域存在内存泄漏的可能性",
      ...
      diagnose: {
        ...
        suggestion: {
          ...
          checkEvacuationFailure: "如果GC出现了To-space exhausted的情况，会大幅拉长GC暂停，解决方法一方面是G1尽量不要设Xmn参数，另一方面考虑排查是否存在大对象分配过多或者内存泄漏或者晋升过快等情况",
